...
class KANConvNDLayer(nn.Module):
    def __init__(self, conv_class, norm_class, input_dim, output_dim, spline_order, kernel_size,
        ...
        self.grid = torch.linspace(
            self.grid_range[0] - h * spline_order,  # 起始点
            self.grid_range[1] + h * spline_order,  # 终止点
            grid_size + 2 * spline_order + 1,  # 取几个点
            dtype=torch.float32
        )
        # Initialize weights using Kaiming uniform distribution for better training start
        # 使用Kaiming均匀分布初始化卷积层的权重，以提高训练起始效果
    ...
    def forward_kan(self, x, group_index):
        ...
        x_uns = x.unsqueeze(-1)  # Expand dimensions for spline operations.  # 扩展维度以进行样条操作
        ...
        # 计算多阶样条的基函数
        for k in range(1, self.spline_order + 1):  # self.spline_order = 3， 即循环三次
    ...
    def forward(self, x):
        ...
        output = []
        for group_ind, _x in enumerate(split_x):  # 等于没切
